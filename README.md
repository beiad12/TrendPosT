...
- **Categories** (`config.ts#CATEGORIES`, 17 total, bilingual AR/EN labels): Breaking, Incidents, Military, Weather, Politics, Sports, Celebrity, Entertainment, **Viral & Reddit**, Life Stories, Economy, Health, Technology, Development, Society, World, General. The dashboard's category chip row (`GET /api/trends/categories`) filters the list by any of them. "Viral & Reddit" is special: a Reddit-sourced story that doesn't match a more specific category (sports/celebrity/politics/etc. still win when they apply) lands here rather than a weak generic bucket — it's how "show me the Reddit stories" works as a filter.
...
- `GET /api/trends/categories` — the full category list the dashboard's filter chips render from.
